  ...
  return (
    <footer className={styles.footer}>
      <div className={styles.container}>
        ...
        <div className={styles.language} ref={menuRef}>
          <button
          ...
          >
            <Globe size={18} />
            {localeNames[locale]}
            <ChevronDown size={16} className={isLangMenuOpen ? styles.chevronUp : ""} />
          </button>
          ...
          {isLangMenuOpen && (
            <div className={styles.menu}>
              {Object.entries(localeNames).map(([key, name]) => (
                <button
                  key={key}
                  className={`${styles.menuItem} ${locale === key ? styles.active : ""}`}
      ...
      <div className={styles.copyright}>
        <p>© 2026 GachaDamn Wiki. Not affiliated with HoYoverse.</p>
      </div>
    </footer>
  );
}
